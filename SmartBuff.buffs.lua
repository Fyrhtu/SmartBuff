...
function SMARTBUFF_InitSpellList()
  ...
  SMARTBUFF_PATTERNS = nil;
  if (SMARTBUFF_CoA_InitClassSpells) then
    SMARTBUFF_CoA_InitClassSpells();
  end

  local buffListClass = SMARTBUFF_PLAYERCLASS;
  if (SMARTBUFF_GetBuffListClass) then
    buffListClass = SMARTBUFF_GetBuffListClass(SMARTBUFF_PLAYERCLASS);
  end
  ...
  if (SMARTBUFF_PLAYERCLASS == "HERO") then
    ...
    if (SMARTBUFF_CoA_AppendClassBuffs) then
      SMARTBUFF_BUFFLIST = SMARTBUFF_CoA_AppendClassBuffs(SMARTBUFF_BUFFLIST, "VENOMANCER");
    end
  end

  -- Druid
  if (buffListClass == "DRUID") then
    SMARTBUFF_BUFFLIST = {      
      {SMARTBUFF_MOTW, 30, SMARTBUFF_CONST_GROUP, {1,10,20,30,40,50,60,70,80}, "WPET;DKPET", SMARTBUFF_GOTW, 60, {50,60,70,80}, {SMARTBUFF_WILDBERRIES,SMARTBUFF_WILDTHORNROOT,SMARTBUFF_WILDQUILLVINE,SMARTBUFF_WILDSPINELEAF}},
      {SMARTBUFF_THORNS, 10, SMARTBUFF_CONST_GROUP, {6,14,24,34,44,54,64,74}, "HUNTER;ROGUE;MAGE;PRIEST;PALADIN;WARLOCK;WPET;DKPET"},
      {SMARTBUFF_BARKSKIN, 0.25, SMARTBUFF_CONST_SELF},
  ...
  -- Venomancer (CoA)
  if (SMARTBUFF_PLAYERCLASS == "VENOMANCER") then
    SMARTBUFF_BUFFLIST = {
      {SMARTBUFF_BLADEFLURRY, 0.165, SMARTBUFF_CONST_SELF},
      {SMARTBUFF_SAD, 0.2, SMARTBUFF_CONST_SELF},
      {SMARTBUFF_EVASION, 0.2, SMARTBUFF_CONST_SELF},
    ...
    SMARTBUFF_PATTERNS = SMARTBUFF_COA_VENOM_PATTERN;
  end

  -- Rogue
  if (buffListClass == "ROGUE") then
    SMARTBUFF_BUFFLIST = {
      {SMARTBUFF_BLADEFLURRY, 0.165, SMARTBUFF_CONST_SELF},
      {SMARTBUFF_SAD, 0.2, SMARTBUFF_CONST_SELF},
      {SMARTBUFF_TRICKS, 0.5, SMARTBUFF_CONST_GROUP, {75}, "WARRIOR;DEATHKNIGHT;DRUID;SHAMAN;HUNTER;ROGUE;MAGE;PRIEST;PALADIN;WARLOCK;HPET;WPET;DKPET"},
  ...
  -- Deathknight
  if (buffListClass == "DEATHKNIGHT") then
    SMARTBUFF_BUFFLIST = {
  ...
  if (SMARTBUFF_BUFFLIST ~= nil and SMARTBUFF_CoA_AppendClassBuffs and SMARTBUFF_IsCoAClass(SMARTBUFF_PLAYERCLASS)
    and SMARTBUFF_PLAYERCLASS ~= "VENOMANCER" and SMARTBUFF_PLAYERCLASS ~= "HERO") then
    SMARTBUFF_BUFFLIST = SMARTBUFF_CoA_AppendClassBuffs(SMARTBUFF_BUFFLIST, SMARTBUFF_PLAYERCLASS);
  end

  if (SMARTBUFF_PLAYERCLASS == "STORMBRINGER" and SMARTBUFF_CoA_MergePatterns) then
    SMARTBUFF_PATTERNS = SMARTBUFF_CoA_MergePatterns(SMARTBUFF_PATTERNS, SMARTBUFF_COA_STORM_PATTERN);
  end

  if (SMARTBUFF_PLAYERCLASS == "CHRONOMANCER" and SMARTBUFF_CoA_MergePatterns) then
    SMARTBUFF_PATTERNS = SMARTBUFF_CoA_MergePatterns(SMARTBUFF_PATTERNS, SMARTBUFF_COA_CHRONO_PATTERN);
  end

  if (SMARTBUFF_PLAYERCLASS == "PRIMALIST" and SMARTBUFF_CoA_MergePatterns) then
    SMARTBUFF_PATTERNS = SMARTBUFF_CoA_MergePatterns(SMARTBUFF_PATTERNS, SMARTBUFF_COA_INSTINCT_PATTERN);
    SMARTBUFF_PATTERNS = SMARTBUFF_CoA_MergePatterns(SMARTBUFF_PATTERNS, SMARTBUFF_COA_BOON_PATTERN);
  end

  if (SMARTBUFF_PLAYERCLASS == "TINKER" and SMARTBUFF_CoA_MergePatterns) then
    SMARTBUFF_PATTERNS = SMARTBUFF_CoA_MergePatterns(SMARTBUFF_PATTERNS, SMARTBUFF_COA_TINKER_PATTERN);
  end
